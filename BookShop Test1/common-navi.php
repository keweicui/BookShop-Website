                    ...
                    <div class="t3-megamenu animate slide" data-duration="400" data-responsive="true">
                        <ul itemscope="" itemtype=""
                            ...
                            <li itemprop="name" data-id="435" data-level="1">
                                <a id="index" itemprop="url" class="" href="index.php" data-target="#">HomePage </a>
                            </li>

<?php
	$sql=mysqli_query($conn,"select * from tb_type order by id asc");//执行查询操作
	$info=mysqli_fetch_array($sql);//将查询结果返回到数组中
 	if($info==false){//如果查询结果为空
		echo "本站暂无商品类别!";//输出字符串
   	}else{
		do{
			if(isset($_GET['id']) && $_GET['id'] == $info['id']){//如果地址栏参数type的值等于查询结果中id字段的值
?>
